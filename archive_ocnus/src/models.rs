...
macro_rules! model_impl_coords {
    ($model:ident, $csst: ty, $coords: ident, $params: expr) => {
        impl<T, G>
            BFMGeometry<T, { $coords::<f32>::NDIMS }, { $coords::<f32>::NPARAMS + $params.len() }>
            for $model<T, G>
        where
            T: Copy + RealField,
        {
            const PARAM_NAMES: SVector<&'static str, { $coords::<f32>::NPARAMS + $params.len() }> =
            ...
            fn transform_internal_to_external<RStride: Dim, CStride: Dim>(
                ...
                $coords::transform_internal_to_external::<RStride, CStride>(
                    ics,
                    &params.fixed_rows::<{ $coords::<f32>::NPARAMS }>(0),
                    cs_state,
                )
            ...
            fn transform_external_to_internal<RStride: Dim, CStride: Dim>(
                ...
                $coords::transform_external_to_internal::<RStride, CStride>(
                    ecs,
                    &params.fixed_rows::<{ $coords::<f32>::NPARAMS }>(0),
                    cs_state,
                )
