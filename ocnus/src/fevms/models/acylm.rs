use crate::{
    fevms::{ForwardEnsembleVectorModel, ModelObserVec},
    math::{bessel_jn, quaternion_xyz},
    model::{OcnusModel, OcnusState},
    scobs::ScConf,
    Fp,
};
use nalgebra::{Const, DimAdd, SVector, ToTypenum, Vector3};
...
where
    M: OcnusModel<P>,
    Const<P>: ToTypenum + DimAdd<Const<1>>,
{
...
macro_rules! impl_acylm {
    ($model: ident, $params: expr, $fn_coords: tt, $fn_obs: tt, $docs: literal) => {
        ...
        #[derive(Debug)]
        pub struct $model<'a>(pub DensityRef<'a, { $params.len() }>);

        impl<'a> OcnusModel<LinearModelState, { $params.len() }> for $model<'a> {
            const PARAMS: [&'static str; { $params.len() }] = $params;

            fn evolve(
                &self,
                time_step: Fp,
                params: &SVector<Fp, { $params.len() }>,
            ...
            fn initialize_states(
                ...
                // Extract parameters using their identifiers.
                let phi = Self::get_param_value("phi", params).unwrap().to_radians();
                let theta = Self::get_param_value("theta", params).unwrap().to_radians();
            ...
            fn prior(&self) -> &DensityRef<{ $params.len() }> {
                &self.0
            }
        }

        impl<'a> EnsblModel<LinearModelState, { $params.len() }> for $model<'a> {
            const RCS: usize = 512;
        }

        impl<'a> ForwardEnsembleVectorModel<LinearModelState, { $params.len() }, 3> for $model<'a> {
            fn generate_vector_observable(
                &self,
                scconf: &ScConf,
                params: &SVector<Fp, { $params.len() }>,
                state: &LinearModelState,
                diag: bool,
            ) -> Option<ModelObserVec<3>> {
                // Extract parameters using their identifiers.
                let phi = Self::get_param_value("phi", params).unwrap().to_radians();
                let theta = Self::get_param_value("theta", params).unwrap().to_radians();
                ...
                match diag {
                    true => Some(ModelObserVec::<3>::from([r, phi, psi])),
                    false => {
                        ...
                        match obs {
                            Some(b_q) => {
                                ...
                                Some(ModelObserVec::<3>::from([b_s[0], b_s[1], b_s[2]]))
                            }
                            None => None,
                        }
                    }
...
mod tests {
    ...
    use super::*;
    use crate::{
        dnsty::{UnivariateDensity, UnivariateType},
        model::{EnsblData, EnsblModel, OcnusModel},
        scobs::{ScConf, ScObs},
    };
    ...
    #[test]
    fn test_acylm_lff() {
        let prior_density = UnivariateDensity([
            UnivariateType::Uniform {
                range: (-90.0, 90.0),
            },
            UnivariateType::Uniform {
                range: (-180.0, 180.0),
            ...
            UnivariateType::Constant { value: 1125.0 },
            UnivariateType::Uniform {
                range: (5.0, 100.0),
            },
            UnivariateType::Uniform { range: (-2.4, 2.4) },
            UnivariateType::Uniform { range: (0.0, 1.0) },
        ]);

        let model = CCLFF_Model(DensityRef::UnivariateRef(&prior_density));

        let sc = ScObs::<ModelObserVec<3>>::from_iterator(
            (0..8).map(|i| ScConf::TimeDistance(224640.0 + i as Fp * 3600.0 * 2.0, 1.0)),
        );

        let mut data = EnsblData::new(1);

        let mut output = ndarray::Array2::<Option<ModelObserVec<3>>>::default((sc.len(), 1));

        data.0[0] = SVector::from([0.0, 0.0, 0.0, 0.2, 600.0, 20.0, 1.0, 0.0]);

        assert!(model
            .initialize_states_ensemble(sc.as_scconf_slice(), &mut data)
            .is_ok());
        assert!(model
            .forward_simulation(
                sc.as_scconf_slice(),
        ...
        assert!(CCLFF_Model::get_param_index("B").unwrap() == 5);
    }
    ...
    fn test_acylm_ut() {
        let prior_density = UnivariateDensity([
            ...
            UnivariateType::Uniform { range: (-5.0, 5.0) },
            UnivariateType::Uniform { range: (0.0, 1.0) },
        ]);

        let model = CCUT_Model(DensityRef::UnivariateRef(&prior_density));
    ...
    fn test_acylm_nc18() {
        let prior_density = UnivariateDensity([
            ...
            UnivariateType::Uniform { range: (0.2, 1.0) },
            UnivariateType::Constant { value: 1125.0 },
            UnivariateType::Uniform {
                range: (5.0, 100.0),
            },
            UnivariateType::Uniform { range: (1.0, 2.0) },
            UnivariateType::Uniform {
                range: (-10.0, 10.0),
            },
            UnivariateType::Uniform { range: (0.0, 1.0) },
        ...
        let model: NC18_Model<'_> = NC18_Model(DensityRef::UnivariateRef(&prior_density));
        ...
        data.0[0] = SVector::from([0.0, 0.0, 0.0, 0.2, 0.5, 600.0, 20.0, 1.25, 0.5, 0.0]);
        ...
        assert!(NC18_Model::get_param_index("B").unwrap() == 6);
    }
}
