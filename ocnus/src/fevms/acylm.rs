use crate::{
    fevms::{FEVMData, FEVMError, FEVM},
    math::{bessel_jn, quaternion_xyz},
    model::{OcnusError, OcnusModel, OcnusState},
    obser::ObserVec,
    scobs::{ScConf, ScObs},
    stats::PDF,
};
...
macro_rules! impl_acylm {
    ($model: ident, $params: expr, $param_ranges:expr, $fn_coords: tt, $fn_obs: tt, $docs: literal) => {
        #[doc=$docs]
        ...
        impl<T> FEVM<LinearModelState, { $params.len() }, 3> for $model<T>
        where
            T: Sync,
            for<'a> &'a T: PDF<{ $params.len() }>,
            Self: OcnusModel<{ $params.len() }>,
        {
            ...
            fn fevm_observe(
                ...
                match obs {
                    Some(b_q) => {
                        ...
                        Ok(ObserVec::<3>::from([b_s[0], b_s[1], b_s[2]]))
                    }
            ...
            fn fevm_state(
                ...
                if !(-1.0..=1.0).contains(&y) {
                    return Err(OcnusError::FEVM(FEVMError::InvalidParameter {
                        name: "y",
                        value: y,
                    }));
                }
