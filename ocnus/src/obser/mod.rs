...
//! # Observables
//!
//! The core item of this module is the `OcnusObser` trait, which must be implemented for any type acting as a model observable.
//!
...
//! - [`sort_by_timestamp`](`ScObsSeries::sort_by_timestamp`) Sorts the underlying vector of [`ScObs`] objects by their timestamps.
//! - [`split`](`ScObsSeries::split`) The reciprocal of one or multiple [`Add`][`std::ops::Add`] calls.
//!   Calling this function consumes a composite [`ScObsSeries`] and returns the original individual [`ScObsSeries`] objects in a vector.
//!
//! An individual [`ScObs`] also stores a spacecraft configuration. The following variants are currently implemented:
