...
fn main() {
    ...
    let prior = UnivariateND::new([
        Uniform1D::new((250.0, 350.0)).unwrap(), // a1 = 285 # speeed
        Uniform1D::new((500.0, 620.0)).unwrap(), // a2 = 625 # speed
        Uniform1D::new((2.1 / 9.0, 2.5 / 9.0)).unwrap(), // a3 = 2/9 # expansion factor coefficient
        Uniform1D::new((0.75, 0.95)).unwrap(),   // a4 = 1 # exp offset
        Uniform1D::new((0.55, 0.7)).unwrap(),    // a5 = 0.8 # exp factor
        Uniform1D::new((1.8, 2.2)).unwrap(),     // a6 = 2 # distance fraction (DO NOT CHANGE)
        Uniform1D::new((1.8, 2.2)).unwrap(),     // a7 = 2 # distance factor coefficient
        Uniform1D::new((2.4, 3.0)).unwrap(),     // a8 = 3 # coefficient everything
    ]);
    ...
    const REF_STEP: usize = 2;
    const ENSEMBLE_SIZE: usize = 512;

    let sc = ScObsSeries::<f32, ObserVec<f32, 1>>::from_iterator((10..200).map(|i| {
        ScObs::new(
