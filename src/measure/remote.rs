use bayesfm::{
    BFEnsblData, BFModel, ModelError, ObsEnsbl,
    noise::ObsNoise,
    observables::{ObsImg, ObsVec},
};
    ...
    Self: BFModel<T, 3, P> + Plasma<T, OC, 3, P>,
{
        ...
        StandardUniform: Distribution<T>,
    {
        ...
        let positions_ecs = Vec::from_par_iter(positions_ics.par_column_iter().map(|ics| {
            Self::transform_internal_to_external::<U1, Const<P>>(&ics.as_view(), params, cs_state)
                .unwrap()
        }));
