use crate::{
    base::{OcnusObser, ScObs, ScObsSeries},
    coords::OcnusCoords,
    stats::Density,
};
use itertools::zip_eq;
use log::debug;
use nalgebra::{Const, RealField, VectorView, VectorViewMut};
use rand::{Rng, SeedableRng};
use rand_distr::{Distribution, StandardNormal, uniform::SampleUniform};
use rand_xoshiro::Xoshiro256PlusPlus;
use rayon::prelude::*;
use std::{marker::PhantomData, ops::AddAssign, time::Instant};
use thiserror::Error;
    ...
    StandardNormal: Distribution<T>,
{
    ...
    fn forward(
        &self,
        time_step: T,
        params: &VectorView<T, Const<P>>,
        fm_state: &mut FMST,
        cs_state: &mut CSST,
    );

    /// Initialize the model parameters, the coordinate system and forward model states.
    fn initialize<const A: usize, D>(
        &self,
        params: &mut VectorViewMut<T, Const<P>>,
        fm_state: &mut FMST,
        cs_state: &mut CSST,
        opt_pdf: Option<&D>,
        rng: &mut impl Rng,
    ) where
        for<'x> &'x D: Density<T, Const<P>>,
    {
    ...
    //                     self.initialize( params, fm_state, cs_state, opt_pdf, &mut rng)?;
    ...
    fn initialize_params<const A: usize>(
        &self,
        params: &mut VectorViewMut<T, Const<P>>,
        opt_pdf: Option<impl Density<T, Const<P>>>,
        rng: &mut impl Rng,
    ) {
        params.set_column(
            0,
    ...
    fn model_prior(&self) -> impl Density<T, Const<P>>;
