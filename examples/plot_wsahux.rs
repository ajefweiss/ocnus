...
fn main() {
    ...
    let sc = ScObsSeries::<f32>::from_iterator((0..(refobs.len() / STEP)).map(|i| {
        ScObs::new(
    ...
    let prior = MultivariateDensity::<_, 8>::new(&[
        ...
        ConstantDensity::new(725.0),     // a2 = 625 # speed
        ConstantDensity::new(2.5 / 9.0), // a3 = 2/9 # expansion factor coefficient
        ConstantDensity::new(1.01),      // a4 = 1 # exp offset
        ConstantDensity::new(0.65),      // a5 = 0.8 # exp factor
        ConstantDensity::new(2.5),       // a6 = 2 # distance fraction (DO NOT CHANGE)
    ...
    model
        .initialize_ensbl::<100, _>(&mut data, None::<&MultivariateDensity<f32, 8>>, 41)
        .unwrap();
    ...
    let t_0 = *sc.first_scobs().unwrap().timestamp();
    let t_1 = *sc.last_scobs().unwrap().timestamp();

    let path = Path::new("examples").join("output").join("plot_wsahux");
    ...
    if let Some(base_dir) = base_dir_opt {
        ...
        chart
            .draw_series(LineSeries::new(
                sc.into_iter()
                    .enumerate()
                    .map(|(idx, scobs)| (*scobs.timestamp(), refobs[idx * STEP])),
                BLACK,
            ))
            .unwrap()
