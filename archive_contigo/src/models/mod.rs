...
macro_rules! impl_atmos_model {
    ($model: ident, $coeffs: expr, $params: expr) => {
        ...
        impl<T, G> BFMGeometry<T, 3, $params> for $model<T, G>
        where
            T: Copy + RealField,
        {
            const PARAM_NAMES: SVector<&'static str, $params> = SVector::from_array_storage(
            ...
            fn transform_internal_to_external<RStride: Dim, CStride: Dim>(
                ...
                CartesianGeometry::transform_internal_to_external(
                    ics,
                    &params.fixed_rows::<{ CartesianGeometry::<f32>::NPARAMS }>(0),
                    cs_state,
                )
            ...
            fn transform_external_to_internal<RStride: Dim, CStride: Dim>(
                ...
                CartesianGeometry::transform_external_to_internal(
                    ecs,
                    &params.fixed_rows::<{ CartesianGeometry::<f32>::NPARAMS }>(0),
                    cs_state,
                )
