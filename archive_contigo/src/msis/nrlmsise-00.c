...
__inline_double sg0(double ex, double *p, double *ap)
{
    return (g0(ap[1], p) + (g0(ap[2], p) * ex + g0(ap[3], p) * ex * ex +
                            g0(ap[4], p) * pow(ex, 3.0) + (g0(ap[5], p) * pow(ex, 4.0) + g0(ap[6], p) * pow(ex, 12.0)) * (1.0 - pow(ex, 8.0)) / (1.0 - ex))) /
           sumex(ex);
}
    ...
    if ((flags->sw[10]) && (input->g_long > -1000.0))
    {
        ...
        if (flags->sw[11])
        {
            t[10] = (1.0 + p[80] * dfa * flags->swc[1]) *
                    ((p[64] * plg[1][2] + p[65] * plg[1][4] + p[66] * plg[1][6] + p[103] * plg[1][1] + p[104] * plg[1][3] + p[105] * plg[1][5] + flags->swc[5] * (p[109] * plg[1][1] + p[110] * plg[1][3] + p[111] * plg[1][5]) * cd14) *
                         cos(dgtr * input->g_long) +
                     (p[90] * plg[1][2] + p[91] * plg[1][4] + p[92] * plg[1][6] + p[106] * plg[1][1] + p[107] * plg[1][3] + p[108] * plg[1][5] + flags->swc[5] * (p[112] * plg[1][1] + p[113] * plg[1][3] + p[114] * plg[1][5]) * cd14) *
